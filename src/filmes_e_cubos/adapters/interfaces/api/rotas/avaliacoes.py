...
from filmes_e_cubos.adapters.interfaces.convencoes import clube_da_sessao
...
@roteador.post(
    ...
    description="A nota precisa respeitar a escala do clube dono da sessão. `nota: null` "
    "registra que o membro cochilou (dorminhoco): o voto fica para a posteridade, mas é "
    "ignorado em médias. Cada membro avalia cada sessão uma vez só.",
)
def avaliar(sessao_id: UUID, nova: NovaAvaliacao, contexto: ContextoDep) -> AvaliacaoSaida:
    ...
    avaliacao = contexto.avaliar_filme.executar(
        ...
        clube_id=clube_da_sessao(sessao, contexto.indicacoes, contexto.rodadas),
